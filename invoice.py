...
class RecoverInvoiceStart(ModelView):
    ...
    pos = fields.Many2One('account.pos', 'Punto de Venta', required=True)
    invoice_type = fields.Many2One('account.pos.sequence', 'Tipo de Comprobante',
...
class RecoverInvoiceFactura(ModelView):
    ...
    message = fields.Text('Mensage', readonly=True)
...
class RecoverInvoice(Wizard):
    ...
    start = StateView(
        ...
        'recover_invoice_ar.recover_invoice_start_view', [
            ...
            Button('Consultar AFIP', 'ask_afip', 'tryton-go-next', default=True),
        ])
    factura = StateView(
        'recover.invoice.factura',
        'recover_invoice_ar.recover_invoice_factura_view', [
            Button('Cerrar', 'end', 'tryton-close'),
            Button('Anterior', 'start', 'tryton-go-previous', default=True),
            Button('Grabar Factura', 'save_invoice', 'tryton-save'),
        ])
